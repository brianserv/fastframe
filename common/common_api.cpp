...
void StringTrim(char *szString)
{
	...
	char szDest[enmMaxStringLength] = { 0 };
	int8_t c = 0;
...
bool IsPupil(const char* szIDCard)
{
	...
	//长度不合法也认为是未成年人
	if (enmIDCardLength - 1 != strlen(szIDCard))
	{
		if (enmIDCardLength2 - 1 != strlen(szIDCard))
		{
			return true;
		}
		...
		temp[0] = '1';
		temp[1] = '9';
		memcpy(temp + 2, szIDCard + 6, 2);
		uint16_t nYear = (uint16_t)atoi(temp);
		if (nYear + 18 > dt.Year())
		{
			return true;
		}
		...
		//判断月份
		memcpy(temp, szIDCard + 8, 2);
		temp[2] = '\0';
		uint16_t nMonth = (uint16_t)atoi(temp);
		...
		//判断日期
		memcpy(temp, szIDCard + 10, 2);
		temp[2] = '\0';
		uint16_t nDate = (uint16_t)atoi(temp);
		if ((1 > nDate) || (31 < nDate))
		{
			return true;
		}
...
void MoveLogFile(int32_t max_file_size, const char* file_name, FILE*& pf)
{
	...
	//获取合适的文件扩展名
	int32_t num = 1;
	for (num = 1; num < enmMaxLogFileNumber; ++num)
	{
...
void StringToPorts(const char* szPorts, char separator, uint16_t& nPortCount, uint16_t arrPorts[])
{
	...
	char str[enmMaxPortStringLength];
	strcpy(str, szPorts);
	char *p = str;
	char *p2 = NULL;
